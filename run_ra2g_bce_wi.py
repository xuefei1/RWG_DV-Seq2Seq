...
from data_loaders import WIA2GBCELoader
from model_a2g import *
...
def main(params):
    params.model_name = "ra2g_bce_wi"
    mutil.DEVICE_STR_OVERRIDE = params.device_str
    data_train = read_col_word_delim_data("data/qrw_wi_train.txt")
    data_valid = read_col_word_delim_data("data/qrw_wi_valid.txt")
    data_test = read_col_word_delim_data("data/qrw_wi_test.txt")
    data_train = extract_wi_pairs_from_qrw_wi_data(data_train)
    data_valid = extract_wi_pairs_from_qrw_wi_data(data_valid)
    data_test = extract_wi_pairs_from_qrw_wi_data(data_test)
    if params.enable_shortlist:
        print("shortlisting data...")
        data_train = data_train[:int(0.5 * len(data_train))]
        data_valid = data_valid[:int(0.1 * len(data_valid))]
        data_test = data_test[:int(0.1 * len(data_test))]
    ...
    pre_built_w2v = None
    src_vocab_cache_file = "cache/wi_src_vocab.pkl"
    tgt_vocab_cache_file = "cache/wi_tgt_vocab.pkl"
    if params.enable_shortlist:
        src_vocab_cache_file = "cache/wi_src_limited_vocab.pkl"
        tgt_vocab_cache_file = "cache/wi_tgt_limited_vocab.pkl"
    ...
    train_loader = WIA2GBCELoader(params.batch_size, src_vocab, tgt_vocab, data_train)
    valid_loader = WIA2GBCELoader(params.batch_size, src_vocab, tgt_vocab, data_valid)
    test_loader = WIA2GBCELoader(params.batch_size, src_vocab, tgt_vocab, data_test)
    ...
    model = make_rnn_based_a2g_model(src_vocab.w2v_mat if params.use_pretrained_embedding else None,
    ...
    if not os.path.isfile(params.saved_model_file) or \
        ...
        print("Training")
        try:
            train_a2g_bce(params, model, train_loader, criterion, model_opt,
    ...
    if len(test_loader) > 0:
        ...
        with torch.no_grad():
            ...
            model.eval()
            # eval_a2g_bce(model, test_loader, params, tgt_vocab.i2w, criterion, k=10, desc="Test")
            run_a2g_bce_epoch(test_loader, model, criterion, None,
